...
class AddPosVar(MovingCameraScene):
    def construct(self):    
            ...
            def redraw_braces():
                ...
                return braces
            braces = always_redraw(redraw_braces)
            ...
            axis_labels = always_redraw(redraw_labels)    
            ...
            vertices_var.arrange_in_grid(rows=1,cell_alignment=[0,-1,0], buff=MED_SMALL_BUFF).next_to(indices_var, DOWN*2).scale(0.75)    
            ...
            def fade_in_vector_brackets():
                ...
                for i in range(vertices_var.__len__()):
                    if (i%2 == 0): 
                        vector_brackets_anim.append(FadeIn(vertices_var[i]))
                return vector_brackets_anim
           
            black_rectangle = Rectangle(color=BLACK, fill_opacity=1 ,height=3,width=13).shift(DOWN*3.25)
            self.add(axis_labels)
            
            self.wait()            
            self.play(self.camera.frame.animate.set(width=20).shift(DOWN)) 
            self.play(FadeIn(black_rectangle,triangles_var,triangles_label,indices_var,indices_var_label, braces))
            ...
            self.play(triangles_label.animate.align_to(vertices_label,RIGHT),
                      ...
                      indices_var.animate.arrange_in_grid(rows=1, cell_alignment=[0,-1,0], buff=MED_LARGE_BUFF*0.97),                      
                      triangles_var.animate.arrange_in_grid(rows=1, cell_alignment=[0,-1,0], buff=LARGE_BUFF*1.66),
                      vertices_var.animate.next_to(indices_var, DOWN*4)
                      )
