...
class MovingAngle(Scene):
    def construct(self):
        ...
        line_moving = Line(ORIGIN, line1.get_end())
        ...
        for t,d in zip(tris,dots):  
            groups.append((AnimationGroup(DrawBorderThenFill(t,run_time=1,rate_func=rush_into),
                                        FadeIn(d, run_time=1.01))))
                                             
      
        self.play(LaggedStart(theta_tracker.animate(run_time=2).increment_value(140),
                            LaggedStart(*(_ for _ in groups),
                                        lag_ratio=0.2),
                            FadeIn(*(_ for _ in dots.copy()),run_time=1),
                                        lag_ratio=0.2),
                        lag_ratio=0.20)
        self.wait(0.5)
